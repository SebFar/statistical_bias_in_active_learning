...
ex = Experiment("MNIST")
ex.observers.append(FileStorageObserver("sacred/radial-16-noisy/overfitting-bias"))
...
def experiment(
    ...
    """
    This experiment is used for figure 4.

    We train models with active learning under various corrections and compute the overfitting bias at each step of training.

    Note that what is described as "R_sure" here is what is written as "R_lure" in the paper.
    """
    ...
    while train_size <= goal_points:
        ...
        if train_size % checkpoints_frequency == 0:
            ...
            for weighting_scheme in ["refined", "naive", "none"]:
                # In this case, we train with the corrected estimator
                new_active_learning_hypers = copy.deepcopy(active_learning_hypers)
                new_active_learning_hypers["weighting_scheme"] = weighting_scheme
                ...
                clean_model = train_to_convergence(
                    ...
                    validation_loader,
                    training_hypers,
                    new_active_learning_hypers,
                    _run,
                    for_acquisition=False,
                )
                # r is roughly clean_test_nll
                clean_test_nll, _, clean_test_accuracy = evaluate(
                    clean_model, test_loader, training_hypers, new_active_learning_hypers
                )
                if weighting_scheme == "refined":
                    _run.log_scalar("sure_test_nll", clean_test_nll, step=train_size)
                elif weighting_scheme == "naive":
                    _run.log_scalar("pure_test_nll", clean_test_nll, step=train_size)
                elif weighting_scheme == "none":
                    _run.log_scalar("unweighted_test_nll", clean_test_nll, step=train_size)

                # then we want \tilde{R}_{SURE}(\theta*) for these weights
                # To that end we calculate the weights as though r_sure, and then do evaluate
                new_active_learning_hypers["weighting_scheme"] = "refined"
                active_learning_data.weighting_scheme = "refined"
                active_learning_data._update_weights(_run)

                r_tilde, r_sure, _ = evaluate(
                    clean_model, train_loader, training_hypers, new_active_learning_hypers
                )
                if weighting_scheme == "refined":
                    _run.log_scalar("sure_unbiased_nll", r_sure, step=train_size)
                    _run.log_scalar("sure_biased_nll", r_tilde, step=train_size)
                elif weighting_scheme == "naive":
                    _run.log_scalar("pure_unbiased_nll", r_sure, step=train_size)
                    _run.log_scalar("pure_biased_nll", r_tilde, step=train_size)
                elif weighting_scheme == "none":
                    _run.log_scalar("unweighted_unbiased_nll", r_sure, step=train_size)
                    _run.log_scalar("unweighted_biased_nll", r_tilde, step=train_size)
